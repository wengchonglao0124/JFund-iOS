...
struct ForgotPaymentPinView: View {
    
    @Environment(\.dismiss) var dismiss
    @GestureState private var dragOffset = CGSize.zero
    
    @State var pinStatus = "new"
    ...
    var forgotPinVM = ForgotPaymentPinViewModel()
                    ...
                    VStack(alignment: .center, spacing: 0) {
                        ...
                        Button {
                            ...
                            if !PasswordService.checkSamePasswords(password1: newPinCode, password2: confirmPinCode) {
                                ...
                                forgotPinVM.requestChangePaymentPin(accessToken: userDataVM.getAccessToken()!, newPin: newPinCode) { success in
                                    if success {
                        ...
                        .background(
                            NavigationLink(destination: EmailVerificationView(navigationTitle: "Forgot payment pin", email: "", serverLocation: "/verify_pincode", isFinished: $isFinish, emailResendVM: forgotPinVM, successMessage: "Payment pin reset successfully!", successSubtitle: "Your payment pin has been reset successfully.", isUsingAccessToken: true, accessToken: userDataVM.getAccessToken()!), tag: "emailVerication", selection: $destinationKey) {
                                EmptyView()
                            }
                        )
                    }
...
struct ForgotPaymentPinView_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            NavigationView {
                ForgotPaymentPinView()
            }
            
            NavigationView {
                ForgotPaymentPinView(pinStatus: "confirm")
            }
        }
    }
}
